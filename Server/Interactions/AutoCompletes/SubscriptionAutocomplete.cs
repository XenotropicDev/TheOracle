...
public class SubscriptionAutocomplete : AutocompleteHandler
{
    public override async Task<AutocompletionResult> GenerateSuggestionsAsync(IInteractionContext context, IAutocompleteInteraction autocompleteInteraction, IParameterInfo parameter, IServiceProvider services)
    {
        try
        {
            ...
            var matches = db.GameContentSets.Where(cs => cs.SetName.Contains(userText));

            successList = matches.Select(cs => new AutocompleteResult(cs.SetName, cs.Id.ToString())).AsEnumerable();
